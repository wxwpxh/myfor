module time_util
  ...
  interface operator(+) ! 讓type(time)型態變數能夠相加
    module procedure add_time_time ! time+time
	module procedure add_time_real ! time+real
	module procedure add_real_time ! real+time
  end interface

  interface operator(<) ! 讓type(time)型態變數能夠比大小
    module procedure time_lt_time  ! 判斷time<time
  end interface

  interface assignment(=) ! 讓type(time)型態跟浮點數能夠轉換
